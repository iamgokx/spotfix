  ...
  const getDepLogs = async () => {
    try {
      const response = await axios.post(
        `http://${API_IP_ADDRESS}:8000/api/admin/getDepNameLogs`
      );
...
const AdminNameLogs = () => {
  ...
  const getDepLogs = async () => {
    try {
      const response = await axios.post(
        `http://${API_IP_ADDRESS}:8000/api/admin/getDepCoordNameLogs`
      );
      if (response.data.status) {
...
const AdminEmailLogs = () => {
  ...
  const getDepLogs = async () => {
    try {
      const response = await axios.post(
        `http://${API_IP_ADDRESS}:8000/api/admin/getDepCoordEmailLogs`
      );
      if (response.data.status) {
