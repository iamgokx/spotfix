...
import govimg from "../../assets/images/admin/government.png";
import GovProposalSuggestionsList from "@/components/GovProposalSuggestionsList";
const DetailedGovProposal = () => {
  const currentTheme = useColorScheme();
  const currentColors = currentTheme == "dark" ? Colors.dark : Colors.light;
  const { govProposalId, suggestions } = useLocalSearchParams();
  ...
  const getDetailedUserProposal = async () => {
    try {
      ...
      const response = await axios.post(
        `http://${API_IP_ADDRESS}:8000/api/proposals/getGovProposalsDetailed`,
        {
  ...
  const openPdfExternally = (fileName: any) => {
    const fileUri = `http://${API_IP_ADDRESS}:8000/uploads/govProposalFiles/${fileName}`;
        ...
        <ScrollView
            ...
            {mediaFiles.length > 0 ? (
              mediaFiles.map((media, index) => (
                <Animatable.Image
                  ...
                  style={styles.img}
                  source={{
                    uri: `http://${API_IP_ADDRESS}:8000/uploads/govProposalMedia/${media}`,
                  }}
                />
              ))
            ) : (
          ...
          <Animatable.View
            ...
            <View>
              <Text
                ...
                {proposalDetails.department_name}
              </Text>
          ...
          <Animatable.View
            ...
            <Text
              ...
              Budget - ₹ {formatBudget(proposalDetails.budget)}
            </Text>
            <Text style={{ color: currentColors.text }}>
              Estimate Time - {proposalDetails.estimated_completion_time}
            </Text>
          </Animatable.View>
...
export default DetailedGovProposal;
